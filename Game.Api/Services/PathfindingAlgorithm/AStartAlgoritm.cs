...
namespace Game.Api.Services.PathfindingAlgorithm
{
    public class AStartAlgoritm
    {
        ...
        public void AddAdjacentCell(Point position, AStartCell parent)
        {
            ...
            var delta = GetTransitionCost(parent.Position, position);
        ...
        {
            return (Math.Abs(first.X - second.X) + Math.Abs(first.Y - second.Y)) * 10;
        }

        private int GetTransitionCost(Point first, Point second)
        {
